...
import com.jwebmp.plugins.bootstrap.progressbar.bar.BSProgressBarThemes;
		...
		extends SB2DropDown
{
			...
			getMessages().forEach(message ->
			                      {
				                      ...
				                      container.add(buildProgressBar(message));
				                      li.add(link);
				                      link.add(container);
	...
	private BSProgressBar buildProgressBar(SB2DropDownTask message)
	{
		...
		progressBar.getProgressBar()
		           .setMax(100);
		progressBar.getProgressBar()
		           .setValue(message.getPercentage());
		progressBar.getProgressBar()
		           .setLabel(Double.toHexString(message.getPercentage()) + " % Complete");
		progressBar.setActive(true);
		progressBar.getProgressBar()
		           .setTheme(BSProgressBarThemes.valueOf(message.getData()));
		return progressBar;
	}
